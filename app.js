...
function initDragFunctions() {
  tiles.forEach((tile) => {
    ...
    tile.addEventListener("drop", function () {
      const isAdjacent =
        draggedTileIndex === dragOverTileIndex - 1 ||
        draggedTileIndex === dragOverTileIndex + 1 ||
        draggedTileIndex === dragOverTileIndex - boardSize ||
        draggedTileIndex === dragOverTileIndex + boardSize;
      if (
        draggedTileIndex !== undefined &&
        dragOverTileIndex !== undefined &&
        draggedTileIndex !== dragOverTileIndex &&
        isAdjacent
      ) {
        const draggedTileIsTitled =
          !tiles[draggedTileIndex].classList.contains("empty-tile");
        const dropTargetIsTitled =
          !tiles[dragOverTileIndex].classList.contains("empty-tile");
        if (
          (draggedTileIsTitled && dropTargetIsTitled) ||
          (!draggedTileIsTitled && !dropTargetIsTitled)
        ) {
          return;
        }
        [tilePositions[draggedTileIndex], tilePositions[dragOverTileIndex]] = [
          tilePositions[dragOverTileIndex],
          tilePositions[draggedTileIndex],
        ];
        renderTiles();
      }
      this.classList.remove("drag-over");
